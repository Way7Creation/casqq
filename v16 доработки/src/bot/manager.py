...
class BotManager:
        ...
        try:
            ...
            for opportunity in sorted_opportunities[:3]:
                try:
                    ...
                    # ✅ ИСПРАВЛЕНО: Правильная проверка режима торговли
                    if config.PAPER_TRADING or config.TEST_MODE or config.BYBIT_TESTNET:
                        ...
                        logger.debug(f"💰 Виртуальный баланс обновлен: ${self.available_balance:.2f}")
                        ...
                        if success:
                            ...
                            # Обновляем реальный баланс (будет обновлен через websocket)
                            position_value = position_size * price
                            self.available_balance -= position_value
                            logger.info(f"💰 Баланс после сделки: ${self.available_balance:.2f}")
                            ...
                            logger.error(f"❌ Не удалось выполнить реальную сделку: {symbol}")
                            # Сохраняем неудачную попытку
                            await self._save_trade_to_db(symbol, trade_data, success=False)
            ...
            if trades_executed > 0:
                logger.info(f"📊 ИТОГО выполнено сделок: {trades_executed}")
                logger.info(f"💰 Оставшийся баланс: ${self.available_balance:.2f}")
    ...
    async def _execute_real_trade(self, trade_data: dict) -> bool:
        """Выполнение реальной сделки через доступные системы"""
        try:
            ...
            # Способ 1: Через execution_engine (приоритет)
            if hasattr(self, 'execution_engine') and self.execution_engine:
                try:
                    ...
                    order_params = {
                        'symbol': symbol,
                        'side': signal.lower(),  # 'buy' или 'sell'
                        'amount': position_size,
                        'order_type': 'market' if config.is_enabled('USE_MARKET_ORDERS') else 'limit',
                        'price': price if not config.is_enabled('USE_MARKET_ORDERS') else None,
                        'stop_loss': trade_data.get('stop_loss'),
                        'take_profit': trade_data.get('take_profit')
                    }
                    
                    # Асинхронное выполнение если поддерживается
                    if hasattr(self.execution_engine, 'execute_order_async'):
                        order = await self.execution_engine.execute_order_async(**order_params)
                    else:
                    ...
                    if order and order.get('status') in ['filled', 'closed']:
                        logger.info(f"✅ Ордер выполнен через execution_engine: {order.get('orderId')}")
                        return True
                    else:
                        logger.warning(f"⚠️ Execution_engine вернул статус: {order.get('status') if order else 'None'}")
            ...
            if hasattr(self, 'enhanced_exchange_client') and self.enhanced_exchange_client:
                try:
                    ...
                    order_result = await self.enhanced_exchange_client.create_order(
                        symbol=symbol,
                        order_type='market' if config.is_enabled('USE_MARKET_ORDERS') else 'limit',
                        side=signal.lower(),
                        amount=position_size,
                    ...
                    if order_result and order_result.get('success'):
                        logger.info(f"✅ Ордер выполнен через enhanced_exchange: {order_result.get('orderId')}")
                        return True
                    else:
