...
class BotManager:
    ...
    async def _execute_real_order(self, symbol: str, signal: str, position_size: float,
        ...
        try:
            ...
            # Способ 1: Через enhanced exchange с V5 API (ПРИОРИТЕТНЫЙ для Bybit)
            if hasattr(self, 'enhanced_exchange_client') and self.enhanced_exchange_client:
                try:
                    logger.debug("📡 Используем enhanced_exchange_client с V5 API...")
                    
                    # Проверяем наличие v5_client
                    if hasattr(self.enhanced_exchange_client, 'v5_client') and self.enhanced_exchange_client.v5_client:
                        logger.info("🚀 Используем Bybit V5 API для размещения ордера")
                        
                        # Подготавливаем параметры для V5 API
                        order_params = {
                        ...
                        if not getattr(config, 'USE_MARKET_ORDERS', True):
                        ...
                        if trade_data.get('stop_loss'):
                            ...
                            if signal.upper() == 'BUY' and stop_loss >= price:
                                stop_loss = price * 0.97  # 3% ниже
                                logger.warning(f"⚠️ Скорректирован stop_loss для BUY: {stop_loss}")
                            elif signal.upper() == 'SELL' and stop_loss <= price:
                                stop_loss = price * 1.03  # 3% выше
                                logger.warning(f"⚠️ Скорректирован stop_loss для SELL: {stop_loss}")
                        ...
                        if trade_data.get('take_profit'):
                            ...
                            if signal.upper() == 'BUY' and take_profit <= price:
                                take_profit = price * 1.06  # 6% выше
                                logger.warning(f"⚠️ Скорректирован take_profit для BUY: {take_profit}")
                            elif signal.upper() == 'SELL' and take_profit >= price:
                                take_profit = price * 0.94  # 6% ниже
                                logger.warning(f"⚠️ Скорректирован take_profit для SELL: {take_profit}")
                            order_params['takeProfit'] = str(take_profit)
                            order_params['tpTriggerBy'] = 'LastPrice'
                        
                        # Выполняем ордер
                        logger.info(f"📤 Отправка ордера с параметрами: {order_params}")
                        order_result = await self.enhanced_exchange_client.v5_client.place_order(**order_params)
                        
                        if order_result and order_result.get('retCode') == 0:
                            order_id = order_result.get('result', {}).get('orderId', 'unknown')
                            logger.info(f"✅ Ордер успешно размещен через V5 API!")
                            logger.info(f"🔖 ID ордера: {order_id}")
                            logger.info(f"📊 Детали: {order_result.get('result', {})}")
                            ...
                            logger.error(f"❌ Ошибка V5 API: {error_msg}")
                            return False
                    
                    # Если V5 недоступен, пробуем стандартный метод place_order
                    elif hasattr(self.enhanced_exchange_client, 'place_order'):
                        logger.info("📡 Используем стандартный place_order метод enhanced_exchange_client")
                        
                        # Проверяем сигнатуру метода
                        import inspect
                        sig = inspect.signature(self.enhanced_exchange_client.place_order)
                        params = list(sig.parameters.keys())
                        
                        # Вариант 1: Метод принимает простые параметры
                        if 'side' in params and 'amount' in params:
                        ...
                        # Вариант 2: Метод требует signal объект
                        elif 'signal' in params:
                            # Импортируем TradingSignal из правильного места
                            try:
                                from src.strategies.base import TradingSignal
                            except ImportError:
                            ...
                            logger.error("❌ Неизвестная сигнатура метода place_order")
                            return False
                        
                        if order_result and ('error' not in str(order_result).lower()):
                            logger.info(f"✅ Ордер выполнен через enhanced_exchange")
                            return True
            ...
            # Способ 2: Через execution_engine
            if hasattr(self, 'execution_engine') and self.execution_engine:
                try:
                    logger.debug("📡 Пробуем execution_engine...")
                    
                    # Импортируем TradingSignal
                    try:
                        from src.strategies.base import TradingSignal
                    except ImportError:
                        from strategies.base import TradingSignal
                    ...
                    trading_signal = TradingSignal(
                        ...
                        stop_loss=trade_data.get('stop_loss'),
                        take_profit=trade_data.get('take_profit'),
                        reason=f"Signal for {symbol} from {trade_data.get('strategy', 'unknown')}",
                        risk_reward_ratio=trade_data.get('risk_reward_ratio'),
                        indicators=trade_data.get('indicators'),
                        source=trade_data.get('strategy', 'unknown')
                    )
                    
                    # Добавляем symbol в market_conditions если его там нет
                    market_conditions = trade_data.get('market_conditions', {})
                    market_conditions['symbol'] = symbol
                    market_conditions['position_size'] = position_size
                    ...
                    if result and hasattr(result, 'status'):
                        if result.status.value in ['completed', 'filled']:
                            logger.info(f"✅ Ордер выполнен через execution_engine: {result.order_id}")
                            return True
                        else:
                            logger.warning(f"⚠️ Execution_engine вернул статус: {result.status.value}")
            ...
            # Способ 3: Через базовый exchange_client
            if hasattr(self, 'exchange_client') and self.exchange_client:
                try:
                    logger.debug("📡 Используем базовый exchange_client...")
                    
                    # Валидация параметров
                    validated_sl = trade_data.get('stop_loss')
                    validated_tp = trade_data.get('take_profit')
                    
                    if validated_sl:
                        if signal.upper() == 'BUY' and validated_sl >= price:
                            validated_sl = price * 0.97
                        elif signal.upper() == 'SELL' and validated_sl <= price:
                            validated_sl = price * 1.03
                    ...
                    order = await self.exchange_client.place_order(
                        ...
                        order_type='market' if getattr(config, 'USE_MARKET_ORDERS', True) else 'limit',
                        params={
                            'stopLoss': validated_sl,
                            'takeProfit': validated_tp
                        }
                    )
                    
                    if order and 'error' not in order:
                        logger.info(f"✅ Ордер выполнен через exchange_client: {order.get('id', 'unknown')}")
                        return True
    ...
    async def _execute_best_trades(self, opportunities: list) -> int:
        ...
        try:
            ...
            for opportunity in sorted_opportunities[:max_trades]:
                ...
                elif testnet and not live_trading:
                    # Режим тестнет - реальные сделки на тестовой бирже
                ...
                elif live_trading and not testnet:
                    # Режим реальной торговли
                    logger.info("💸 РЕЖИМ LIVE TRADING - реальная сделка на основной бирже")
                    success = await self._execute_real_order(symbol, signal, position_size, price, trade_data)
                else:
                    # Неопределенный режим - используем безопасный вариант
                    logger.warning("⚠️ Неопределенный режим торговли, используем PAPER TRADING")
                    success = await self._simulate_trade(symbol, signal, position_size, price, trade_data)
